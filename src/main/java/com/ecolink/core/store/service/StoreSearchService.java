...
public class StoreSearchService {
	...
	public CursorPage<StoreSearchDto, Long> searchStores(StoreSearchRequest request, Long avatarId) {
		List<StoreSearchDto> searchDtos = storeJpaRepository.findStoresByKeyword(request, avatarId);

		// 매장 제품 3개만 잘라서 넣기
		storeProductService.addTop3ProductsToDto(request, searchDtos);

		// 이벤트 발행

		return CursorPage.of(searchDtos, request.getSize(), StoreSearchDto::getId);
	}
}
